...
public class Planning {
    ...
    public String toString(){
		// Initialisation de la chaine de caractères à retourner
		String res = new String();

		// On liste les créneaux de l'emploi du temps
		for (Creneau c : edt){
	    	res = res + c + "\n";
		}
		return res;
	...
	/** Vérifie si le créneau c est compatible avec les créneaux déjà insérés dans l'emploi du temps
	 * @param c : le créneau à vérifier
	*/
    public boolean verifCreneau(Creneau c){
		boolean ok = true;
		int i = 0;

		// On parcourt la liste des créneaux déjà insérés dans l'emploi du temps
		while (ok && i < edt.size()){
	...
	/**
	 * Ajoute le créneau c à l'emploi du temps si il est compatible avec les créneaux déjà insérés
	 * @param c
	 */
    ...
    public List<Creneau> planningGroupe(Groupe groupe){
		...
		while (it.hasNext()){
	    	Creneau c = it.next();
	    	if (c.getActivite().getGroupes().contains(groupe)) plan.add(c);
		}
		return plan;
    }
    ...
    public float totalHeuresGroupe(Groupe groupe){
		...
		while (it.hasNext()){
	    	Creneau c = it.next();
	    	if (c.getActivite().getGroupes().contains(groupe)) res += c.getDuree();
		}
		return (float) res;
    }
